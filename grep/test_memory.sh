...
check_flags(){
    ...
    for flag in ${flags[@]}; do
        ...
        valgrind --tool=memcheck --leak-check=yes ./s21_grep $sample $flag $test $test2 > $s21
    ...
    # флаг e
    valgrind --tool=memcheck --leak-check=yes ./s21_grep "-e" $sample $test $test2 > $s21
    ...
    # флаг f
    valgrind --tool=memcheck --leak-check=yes ./s21_grep "-f" $patterns $test $test2 > $s21
...
check_flags_together(){
    ...
    for flag_one in ${flags[@]}; do
        for flag_two in ${flags[@]}; do
            if [ "$flag_one" != "$flag_two" ]; then
                valgrind --tool=memcheck --leak-check=yes ./s21_grep $flag_one $flag_two $sample $test $test2 > $s21
    ...
    for flag in ${flags[@]}; do
        valgrind --tool=memcheck --leak-check=yes ./s21_grep "-e" $sample $flag $test $test2 > $s21
    ...
    for flag in ${flags[@]}; do
        valgrind --tool=memcheck --leak-check=yes ./s21_grep "-f" $patterns $flag $test $test2 > $s21
